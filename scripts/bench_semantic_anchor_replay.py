...
def measure(
    payloads: Iterable[dict[str, Any]], started: float, clock=time.perf_counter
) -> dict[str, Any]:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError(
            "base-url must be a loopback HTTP origin, without a path or credentials"
        )
    ...
    headers = {
        ...
        "X-MTPLX-Client": "semantic-anchor-replay",
    ...
    for index, body in enumerate(bodies):
        ...
        if index == 0 and row["cached_tokens"] != 0:
            raise ValueError(
                "First turn was cached; restart with an isolated empty session bank"
            )
        rows.append(
            {
                "turn": index,
...
def compare(off: dict[str, Any], on: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        left.pop("anchors_enabled") is not False
        or right.pop("anchors_enabled") is not True
    ):
        raise ValueError("Expected OFF receipt followed by ON receipt")
    if left != right:
        raise ValueError(
            "Arms differ in reproducibility metadata beyond the anchor flag"
        )
    ...
    for a, b in zip(before, after):
        ...
        parity = all(
            a[key] == b[key]
            for key in ("output_sha256", "finish_reason", "completion_tokens")
        )
        rows.append(
            {
                "turn": a["turn"],
    ...
    return {
        "schema_version": 1,
        "kind": "paired_http_comparison",
        "output_parity": all(row["output_parity"] for row in rows),
        "performance_claim_requires_repeated_isolated_runs": True,
        "turns": rows,
    }
...
def main() -> int:
    ...
    try:
        ...
        result = (
            run(args)
            if args.command == "run"
            else compare(read_json(args.off), read_json(args.on))
        )
