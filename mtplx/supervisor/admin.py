...
from .proxy import EngineUnavailable, InsufficientMemory, is_authorized, is_localhost_bind
...
def _error(status: int, code: str, message: str | None = None, **extra: Any) -> JSONResponse:
...
class AdminApp:
    ...
    async def __call__(self, scope, receive, send) -> None:
        ...
        if path == "/health":
            ...
            response = await self._list_models(request)
        elif path == "/mtplx/admin/models" and method == "GET":
            response = await self._admin_models(request)
        elif path == "/mtplx/admin/load" and method == "POST":
            response = await self._load(request)
        elif path == "/mtplx/admin/unload" and method == "POST":
            response = await self._unload(request)
        elif path == "/mtplx/admin/restart" and method == "POST":
            response = await self._restart(request)
        elif path == "/mtplx/admin/health" and method == "GET":
            response = await self._admin_health(request)
        else:
    ...
    def _admin_authorized(self, request: Request) -> JSONResponse | None:
        """Returns a 401 response if unauthorized, else None."""
    ...
    def _inference_authorized(self, request: Request) -> JSONResponse | None:
        config = self.supervisor.config
    ...
    async def _health(self, request: Request) -> JSONResponse:
        unauthorized = self._inference_authorized(request)
    ...
    async def _admin_models(self, request: Request) -> JSONResponse:
        unauthorized = self._admin_authorized(request)
        if unauthorized is not None:
            return unauthorized
    ...
    async def _admin_health(self, request: Request) -> JSONResponse:
        unauthorized = self._admin_authorized(request)
        if unauthorized is not None:
            return unauthorized
    ...
    async def _unload(self, request: Request) -> JSONResponse:
        unauthorized = self._admin_authorized(request)
        if unauthorized is not None:
            return unauthorized
        model_id = await self._body_model_id(request)
