...
_MAX_KEY_SPLITS = 64
#: Matches mtplx.runtime_options.QSA_SPARSE_DECODE_DEFAULT_SPLITS; a
...
def qsa_sparse_gqa_decode_split_geometry(
    ...
    arithmetic.  ``tests/test_qsa_sparse_decode.py`` pins the two
...
def qsa_sparse_gqa_decode(
    ...
    *,
    ...
    on the same terms as ``MTPLX_QWEN4_HC_M4``: greedy-token agreement plus a
...
def _ple_cpu_rows_abi_mismatch() -> str | None:
    ...
    A matching build and import that still cannot cast an ``mx::array``. Named
    here so the reason is printed rather than surfacing as a bare ``TypeError``
    at first call.
    """
