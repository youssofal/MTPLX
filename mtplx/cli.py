...
def _add_paged_kv_quant_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        metavar="{off,q8,q6,q4}",
        type=_kv_quant_arg,
        default=None,
        help=(
            "Paged KV cache quantization mode. off is default; q8/q6/q4 opt into "
            ...
            "the threshold. q6 and q4 never kernel and keep no mirror: smaller "
            "KV bytes at every length, decode re-dequantizes per step (slower "
            "long-context decode). Prefill runs unquantized (peak prefill "
            "memory unchanged) and compiled-verify/dense-two-pass fast paths "
            "detach while active. q6 sits between q8 and q4 in storage and "
