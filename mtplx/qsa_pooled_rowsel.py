...
after the model weights and fixed-M4 stack are installed and before graph warmup.
...
def _validate_normal_opdiet() -> dict[str, bool]:
    ...
    rope = bool(options.qwen4_opdiet_enabled("rope"))
    bank = bool(options.qwen4_opdiet_enabled("bank"))
    if not rope or not bank:
        raise FixedM4PoolInstallError(
            "fixed-M4 pool requires the qwen4 op-diet rope and bank items "
            "(MTPLX_QWEN4_OPDIET)"
        )
