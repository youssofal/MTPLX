...
def _opencode_memory_env_defaults() -> dict[str, str]:
    ...
    per_session_bytes = _OPENCODE_HIGH_MEMORY_PER_SESSION_BYTES if high_memory else "4G"
    max_entries = (
...
def _cmd_bench_compare(args: Any) -> int:
    if getattr(args, "matrix", False):
        ...
        payload = run_comparison_matrix(
            affine_model=getattr(args, "affine_model", None),
            nvfp4_model=getattr(args, "nvfp4_model", None),
            model=getattr(args, "model", None),
            sampler_profile_name=getattr(args, "sampler_profile", "mtplx-optimized"),
            prompt_tokens=512,
            max_tokens=int(getattr(args, "max_tokens", 512) or 512),
        ...
        if output:
            write_comparison_matrix(output, payload)
        _print(payload)
        return 0 if payload.get("valid", True) else EXIT_STRICT_GATE
...
def _active_mlx_fork_status(
    *, expected_fragment: str, expected_commit: str | None
    ...
    ok = path_active and (expected_commit is None or commit in {None, expected_commit})
    return {
        "ok": ok,
...
def _quickstart_opencode_payload(
    ...
    *,
    ...
    default_tool_prompt_mode = str(
        OPENCODE_FAIR_BATCHING_DEFAULTS.get("tool_prompt_mode") or "hybrid"
    )
    tool_prompt_mode = (
        str(
...
def cmd_config_public(args: Any) -> int:
    ...
    if key == "scheduler_mode" and value not in {
        ...
        raise SystemExit(
            "scheduler_mode must be serial, cooperative, ar_batch, or mtp_cohort_experimental"
        )
