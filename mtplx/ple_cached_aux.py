...
class _CachedFixedM4SidecarAux:
    ...
    def __call__(
        ...
        # Upstream's fixed-M4 aux (mtplx/qwen4_fixed_verify._FixedM4SidecarAux)
        # is a pure producer: warm ownership lives on the _SidecarGather, which
        # warms itself (qwen4_exp.py _submit_warm), so the aux no longer carries
        # a pending-warm / install-owned-rows step. This lane only replaces plane
        # production; hit/miss routing may differ from the stock owner cache but
        # the gathered row bytes are identical, so the plane stays exact.
        stock = self._stock
        try:
