...
The 15 tests cover stream parsing, first-token attribution, missing/invalid usage, tool-only output, hash stability, comparison drift/parity, atomic non-overwriting receipts, and a loopback fake-server request replay. They establish the measurement contract only. The [September 7 real-model receipt](semantic-anchor-20260907/README.md) now contains all three isolated pairs per workload. Every paired cached-token delta is zero, and the long plain append fails output parity in all three pairs with stable outputs within each arm. An earlier host-contaminated arm remains excluded. The repetition requirement is complete; the empirical acceptance gate fails, and these observations do not support a speed claim.
