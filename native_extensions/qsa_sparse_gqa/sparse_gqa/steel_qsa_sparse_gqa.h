...
//
// Direct-index sparse GQA attention for Qwen3.8 Flash-Next QSA.
//
// Ported into MTPLX from oMLX (jundot/omlx, commit 7467dce8,
// `omlx/custom_kernels/glm_moe_dsa/csrc/kernels/steel_qwen4_qsa_sparse_gqa.h`),
// written by Jonathan Spangler, Apache-2.0, Copyright (c) 2026 OpenAI.
// The 128-bit global K/V staging pattern is in turn adapted from mlx-serve's
// MIT `msv_attn_p256` kernel (Copyright 2026 David Dalcu); the query-specific
// direct-index/GQA organisation and the Steel MMA implementation are oMLX's.
//
// MTPLX changes, all mechanical:
//   * renamed to the mtplx_* symbol space so the metallib can co-exist with
//     an oMLX build in one process;
//   * the host/device parameter block moved into a single shared header
//     (qsa_sparse_gqa_params.h) instead of being declared twice;
//   * `IndexT` is instantiated for int32 as well as uint32, because MTPLX's
//     `_select_eager` emits int32 block ids and an astype would copy 8 MB
//     per layer per 4,096-row chunk;
//   * `params->kL` is the LOGICAL key length supplied by the caller, not the
//     K array's extent: MTPLX hands the kernel the full KV cache backing
//     [1, 2, capacity, 256] and attends to the first `total_tokens` rows.
//
// Algorithm (unchanged): one threadgroup owns one (query row, KV head).  The
// twelve query heads sharing that KV head are padded to a single 16-row Steel
// MMA tile, so both GQA groups reuse each randomly addressed K/V tile without
// materialising a [query, selected, kv-head, dim] gathered tensor.  The
// selected block list is chronological; the zero-to-three-token causal tail is
// generated in-kernel; invalid early-prefix slots are masked to true -inf
// before an fp32 online softmax (exp2).  No score tensor, no gathered K/V, no
// bool mask.
...
struct MtplxQsaMaxOp {
  template <typename T> METAL_FUNC static constexpr T apply(T x, T y) {
    return metal::max(x, y);
  }
};

struct MtplxQsaSumOp {
  template <typename T> METAL_FUNC static constexpr T apply(T x, T y) {
    return x + y;
  }
};

struct MtplxQsaMulOp {
  template <typename T> METAL_FUNC static constexpr T apply(T x, T y) {
    return x * y;
  }
};

struct MtplxQsaExpSubOp {
  template <typename T> METAL_FUNC static constexpr T apply(T x, T y) {
    return fast::exp2(x - y);
  }
};

struct MtplxQsaDivOp {
  template <typename T> METAL_FUNC static constexpr T apply(T x, T y) {
    return x / y;
  }
};
...
mtplx_qsa_sparse_gqa_attention(
    const device T* Q [[buffer(0)]],
    const device T* K [[buffer(1)]],
    const device T* V [[buffer(2)]],
    const device IndexT* Topk [[buffer(3)]],
    device T* O [[buffer(4)]],
    const constant MtplxQsaSparseGqaParams* params [[buffer(5)]],
  ...
  for (int ktile = 0; ktile < n_tiles; ++ktile) {
    ...
    Stile.template row_reduce<MtplxQsaMaxOp>(new_max);
    Stile.template row_bin_op<MtplxQsaExpSubOp>(new_max);
    STEEL_PRAGMA_UNROLL
    for (short i = 0; i < rows_per_thread; ++i) {
    ...
    Stile.template row_reduce<MtplxQsaSumOp>(sum_score_tmp);
    STEEL_PRAGMA_UNROLL
    for (short i = 0; i < rows_per_thread; ++i) {
      sum_score[i] = sum_score[i] * factor[i] + sum_score_tmp[i];
    }
    Otile.template row_bin_op<MtplxQsaMulOp>(factor);
  ...
  Otile.template row_bin_op<MtplxQsaDivOp>(sum_score);
