...
def _stock_rows_matrices_oracle():
    ...
    Upstream main's ``_SidecarGather._rows_matrices`` inlines the hot-row stack
    (``np.stack([row[j] for row in rows])[inverse]``). PR #391 factored that
    into a top-level ``_stack_hot_rows`` helper it documented as bit-identical
    to ``np.stack``; upstream does not carry that helper, so the oracle extracts
    ``_rows_matrices`` alone. For the small mixed hit/miss gather this test
    drives, ``_rows_matrices`` takes the hot-row LRU branch, which touches only
    the attributes the fake ``_Sidecar`` provides (the vectorized branch, which
    imports ``mtplx.ple_row_gather``, is not reached).
    """
    ...
    namespace = {"np": np}
    module = ast.Module(body=[rows_matrices], type_ignores=[])
