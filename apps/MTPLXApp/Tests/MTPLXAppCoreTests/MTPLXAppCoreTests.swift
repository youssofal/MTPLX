...
final class MTPLXAppCoreTests: XCTestCase {
    ...
    func testExternalMlxServePortOccupantIsNeverAdoptedOrLaunchedOver() async throws {
        ...
        XCTAssertTrue(health.ok)
        let settled = await PortPreflight.classifySettled(
            baseURL: baseURL,
            apiKey: nil,
            backendKind: .externalMlxServe,
            settleTimeoutSeconds: 0
        )
        guard case .externalMlxServeServer(let settledHealth) = settled else {
            XCTFail("settled preflight must preserve the external backend, got \(settled)")
            return
        }
        XCTAssertTrue(settledHealth.ok)
