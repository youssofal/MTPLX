...
final class HermesEmbeddedRuntimeTests: XCTestCase {
    ...
    func testSidecarUsesCallerTokenAndRemovesOwnershipRecordOnStopWithoutChangingProfileFiles() async throws {
        ...
        while (sidecar.isRunning
            || FileManager.default.fileExists(atPath: sidecar.ownershipRecordURL.path))
            && Date() < stopDeadline {
    ...
    func testSidecarStopReturnsBeforeEscalationWaitCompletes() async throws {
        let binary = try makeUncooperativeSidecarBinary()
        let process = Process()
        ...
        try process.run()
        _ = output.fileHandleForReading.readData(ofLength: 1)

        let recordURL = sidecarRuntimeDirectory.appendingPathComponent("uncooperative.json")
        try FileManager.default.createDirectory(
            at: sidecarRuntimeDirectory,
            withIntermediateDirectories: true
        )
        try Data("owned".utf8).write(to: recordURL)
        let sidecar = HermesSidecar(
            process: process,
            port: 45123,
            token: "test-token",
        ...
        while (sidecar.isRunning || FileManager.default.fileExists(atPath: recordURL.path))
            && Date() < deadline {
    ...
    func testStartupFailurePreservesNonSecretEnvironmentDiagnostics() async throws {
        let fixture = root.appendingPathComponent("hermes-diagnostic-fixture.sh")
        try """
        #!/bin/sh
        printf 'diagnostic=%s api_key=%s\n' "$MTPLX_FIXTURE_DIAGNOSTIC" "$OPENAI_API_KEY" >&2
        exit 1
        ...
        let failingIntegration = HermesIntegration(
            ...
            environment: [
                "HOME": root.path,
                "PATH": "/usr/bin:/bin",
                "MTPLX_FIXTURE_DIAGNOSTIC": "mode=1 model=current-model",
            ],
            sidecarRuntimeDirectory: sidecarRuntimeDirectory
        )
        ...
        do {
            ...
            let diagnostic = error.localizedDescription
            XCTAssertTrue(diagnostic.contains("diagnostic=mode=1 model=current-model"))
            XCTAssertFalse(diagnostic.contains("test-api-key"))
            XCTAssertTrue(diagnostic.contains("api_key=[redacted]"))
        }
    }

    func testStartupFailureRedactsCredentialSuffixesAndCredentialURLsAcrossWrites() async throws {
        let fixture = root.appendingPathComponent("hermes-credential-diagnostic-fixture.sh")
        try """
        #!/bin/sh
        printf 'diagnostic=%s plain_url=%s secret=' "$MTPLX_FIXTURE_DIAGNOSTIC" "$PUBLIC_STATUS_URL" >&2
        printf '%s' "${SERVICE_SECRET%value}" >&2
        printf '%s credential=' "${SERVICE_SECRET#alpha-secret-}" >&2
        printf '%s' "${SERVICE_CREDENTIAL%value}" >&2
        printf '%s dsn=' "${SERVICE_CREDENTIAL#beta-credential-}" >&2
        printf '%s' "${DATABASE_DSN%@example.invalid/app}" >&2
        printf '%s credential_url=' "${DATABASE_DSN#postgres://dbuser:dbpass}" >&2
        printf '%s' "${SERVICE_URL%@example.invalid/path}" >&2
        printf '%s query_url=' "${SERVICE_URL#https://urluser:urlpass}" >&2
        printf '%s' "${CALLBACK_URL%querysecret}" >&2
        printf '%s\n' "${CALLBACK_URL#https://example.invalid/callback?access_token=}" >&2
        exit 1
        """.write(to: fixture, atomically: true, encoding: .utf8)
        try FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: fixture.path)
        ...
        let secrets = [
            "alpha-secret-value",
            "beta-credential-value",
            "postgres://dbuser:dbpass@example.invalid/app",
            "https://urluser:urlpass@example.invalid/path",
            "https://example.invalid/callback?access_token=querysecret",
        ]
        let plainURL = "https://example.invalid/diagnostic?mode=1"
        let failingIntegration = HermesIntegration(
            hermesHome: hermesHome,
            executablePath: fixture.path,
            environment: [
                ...
                "PATH": "/usr/bin:/bin",
                "MTPLX_FIXTURE_DIAGNOSTIC": "mode=1 model=current-model",
                "SERVICE_SECRET": secrets[0],
                "SERVICE_CREDENTIAL": secrets[1],
                "DATABASE_DSN": secrets[2],
                "SERVICE_URL": secrets[3],
                "CALLBACK_URL": secrets[4],
                "PUBLIC_STATUS_URL": plainURL,
            ],
            sidecarRuntimeDirectory: sidecarRuntimeDirectory
        )
        ...
        do {
            ...
            let diagnostic = error.localizedDescription
            XCTAssertTrue(diagnostic.contains("diagnostic=mode=1 model=current-model"))
            XCTAssertTrue(diagnostic.contains("plain_url=\(plainURL)"))
            XCTAssertTrue(diagnostic.contains("[redacted]"))
            for secret in secrets {
                XCTAssertFalse(diagnostic.contains(secret), "Leaked secret: \(secret)")
            }
            for secretFragment in ["alpha-secret-", "beta-credential-", "dbpass", "urlpass", "querysecret"] {
                XCTAssertFalse(diagnostic.contains(secretFragment), "Leaked secret fragment: \(secretFragment)")
            }
        }
    }
    ...
    private func makeUncooperativeSidecarBinary() throws -> URL {
        let source = root.appendingPathComponent("hermes-uncooperative-fixture.c")
        let binary = root.appendingPathComponent("hermes-uncooperative-fixture")
        try """
        #include <signal.h>
        #include <stdio.h>
        #include <unistd.h>
        ...
        guard compiler.terminationStatus == 0 else {
            throw NSError(domain: "HermesEmbeddedRuntimeTests", code: 1, userInfo: [
                NSLocalizedDescriptionKey: "Could not compile uncooperative sidecar fixture.",
            ])
        }
        return binary
    }
