...
struct HermesPanel: View {
                ...
                ScrollView {
                    VStack(spacing: 6) {
                        ForEach(hermes.sessions) { session in
                            ...
                            .buttonStyle(.plain)
                            .disabled(sessionIsExternallyActive(session))
                            .help(sessionHelp(session))
                            .accessibilityLabel("\(session.title.isEmpty ? "Untitled Agent" : session.title), \(activityLabel(session.activity))")
                            .accessibilityHint(sessionHelp(session))
                        }
                    }
                    .padding(.bottom, 4)
                }
    ...
    private func activityLabel(_ activity: HermesSessionActivityState) -> String {
        ...
        case .externallyActive(let surface): "Active in \(surface)"
        case .ownershipUnknown: "Ownership unknown"
        }
    }

    private func sessionIsExternallyActive(_ session: HermesSavedSession) -> Bool {
        if case .externallyActive = session.activity {
            return true
        }
        return false
    }

    private func sessionHelp(_ session: HermesSavedSession) -> String {
        if case .externallyActive(let surface) = session.activity {
            return "Active in \(surface). Create a new Hermes agent to work here."
        }
        return "Resume \(session.title.isEmpty ? "Untitled Agent" : session.title)"
    }

    private func sessionActivityExplanation(_ activity: HermesSessionActivityState) -> String? {
        if case .externallyActive(let surface) = activity {
            return "This agent is active in \(surface). Create a new agent to work here."
        }
        return nil
    }
    ...
    private func sessionRow(_ session: HermesSavedSession) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if let explanation = sessionActivityExplanation(session.activity) {
                Text(explanation)
                    .font(.caption)
                    .foregroundStyle(Brand.typeTertiary)
                    .lineLimit(2)
