...
public final class HermesSidecar: HermesSidecarControlling, @unchecked Sendable {
    private static let terminationQueue = DispatchQueue(
        label: "com.mtplx.hermes-sidecar-termination",
        qos: .utility,
    ...
    public func stop() {
        ...
        let needsAsynchronousWait = process.isRunning
        stopLock.unlock()

        guard needsAsynchronousWait else {
            removeRecoveryRecordAfterExit()
            return
        }

        Self.terminationQueue.async { [self] in
    ...
    private func removeRecoveryRecordAfterExit() {
...
public struct HermesIntegration: Sendable {
    ...
    public func installStatus() async -> HermesInstallStatus {
        ...
        guard chatHelp.contains("--query") && chatHelp.contains("--source") else {
            return .incompatible(
        ...
        let serveHelp = (try? await runAndCapture(
            executableURL: executable,
            arguments: ["serve", "--help"]
        )) ?? ""
        guard Self.serveHelpSupportsEmbeddedSidecar(serveHelp) else {
            return .incompatible(
                executablePath: executable.path,
                versionSummary: versionSummary,
                detail: "Hermes must support isolated loopback sidecars before MTPLX can embed profile sessions."
            )
        }

        guard let projectURL = Self.projectURL(fromVersionOutput: output),
              Self.projectSupportsAuthenticatedGatewayWebSocket(projectURL)
        else {
            return .incompatible(
                executablePath: executable.path,
                versionSummary: versionSummary,
                detail: "Hermes must expose the authenticated /api/ws gateway before MTPLX can embed profile sessions."
            )
        }
    ...
    private static func stderrSecrets(for spec: HermesServeLaunchSpec) -> [String] {
        let credentialValues = spec.environment.compactMap { key, value in
            (isSecretEnvironmentKey(key) || isCredentialBearingURL(value)) ? value : nil
        }
        return Array(Set(credentialValues + [spec.token]))
            .filter { !$0.isEmpty }
    }

    private static func isSecretEnvironmentKey(_ key: String) -> Bool {
        let normalized = key.uppercased()
        let exactSecretKeys: Set<String> = [
            "API_KEY", "TOKEN", "PASSWORD", "PASSWD", "PRIVATE_KEY",
            "ACCESS_KEY", "CLIENT_SECRET", "SECRET", "CREDENTIAL", "DSN",
            "AUTHORIZATION",
        ]
        return exactSecretKeys.contains(normalized)
            || normalized.hasSuffix("_API_KEY")
            || normalized.hasSuffix("_TOKEN")
            || normalized.hasSuffix("_PASSWORD")
            || normalized.hasSuffix("_PASSWD")
            || normalized.hasSuffix("_PRIVATE_KEY")
            || normalized.hasSuffix("_ACCESS_KEY")
            || normalized.hasSuffix("_CLIENT_SECRET")
            || normalized.hasSuffix("_SECRET")
            || normalized.hasSuffix("_CREDENTIAL")
            || normalized.hasSuffix("_DSN")
    }

    private static func isCredentialBearingURL(_ value: String) -> Bool {
        ...
        if components.user?.isEmpty == false || components.password?.isEmpty == false {
            return true
        }
        return components.queryItems?.contains(where: { isSecretEnvironmentKey($0.name) }) == true
    }
    ...
    private static func projectURL(fromVersionOutput output: String) -> URL? {
        for line in output.split(separator: "\n") {
            ...
            for label in ["Project:", "Install directory:"] where text.hasPrefix(label) {
                let path = text
    ...
    private static func projectSupportsAuthenticatedGatewayWebSocket(_ projectURL: URL) -> Bool {
        let transportURL = projectURL.appendingPathComponent("tui_gateway/ws.py")
        let serverURL = projectURL.appendingPathComponent("hermes_cli/web_server.py")
        guard let transportSource = try? String(contentsOf: transportURL, encoding: .utf8),
              let serverSource = try? String(contentsOf: serverURL, encoding: .utf8)
        else { return false }

        let sessionTokenSource = [
        ...
        let tokenValidation = [
            ...
            "if not token:",
            "hmac.compare_digest(token.encode(), _SESSION_TOKEN.encode())",
            #"return None, "token""#,
        ]
        let authWrapper = [
            "def _ws_auth_ok(",
        ...
        guard let routeSource = sourceBlock(
            startingAt: #"@app.websocket("/api/ws")"#,
            in: serverSource
        ) else { return false }
        ...
        return transportSource.contains("async def handle_ws(")
            && containsInOrder(sessionTokenSource, in: serverSource)
            && containsInOrder(tokenValidation, in: serverSource)
            && containsInOrder(authWrapper, in: serverSource)
            && containsInOrder(authenticatedRoute, in: routeSource)
    }

    private static func sourceBlock(startingAt marker: String, in text: String) -> String? {
        guard let markerRange = text.range(of: marker) else { return nil }
        let remainder = markerRange.upperBound..<text.endIndex
        let end = text.range(of: "\n@app.", range: remainder)?.lowerBound ?? text.endIndex
        return String(text[markerRange.lowerBound..<end])
    }

    private static func containsInOrder(_ needles: [String], in text: String) -> Bool {
        var start = text.startIndex
        for needle in needles {
            guard let range = text.range(of: needle, range: start..<text.endIndex) else {
                return false
            }
    ...
    private static func serveHelpSupportsEmbeddedSidecar(_ help: String) -> Bool {
        ["--isolated", "--host", "--port", "--ssh-owner-nonce"]
            .allSatisfy { help.contains($0) }
    }
