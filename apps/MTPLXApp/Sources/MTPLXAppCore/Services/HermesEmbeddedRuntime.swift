...
enum HermesOrphanSidecarScanner {
    ...
    /// Startup has stronger evidence than recovery: a Python console
    /// entrypoint must canonicalize to the executable selected for this
    /// launch. This keeps an unrelated local `hermes` script from becoming an
    /// ownership record while allowing PATH and symlink-based installations.
    static func isVerifiedPostLaunchIdentity(
        ...
        let canonicalEntrypoint = URL(fileURLWithPath: entrypoint)
            .standardizedFileURL
            .resolvingSymlinksInPath()
        let canonicalExecutable = spec.executableURL
            .standardizedFileURL
            .resolvingSymlinksInPath()
        return canonicalEntrypoint == canonicalExecutable
    }
